...
@pytest.fixture()
def ms2_peaks():
    ms2 = deimos.load(localfile("resources/isotope_example_data.h5"), key="ms2")
    peaks = deimos.peakpick.persistent_homology(
        ms2, dims=["mz", "drift_time", "retention_time"], radius=[2, 10, 0]
    )
    return deimos.threshold(peaks, threshold=1e3)


def test_match(ms1_peaks):
    a, b = deimos.alignment.match(
        ms1_peaks,
        ms1_peaks,
    ...
    # one-to-one check
    assert len(a.index) == len(
        a.drop_duplicates(subset=["mz", "drift_time", "retention_time"]).index
    )
    assert len(b.index) == len(
        b.drop_duplicates(subset=["mz", "drift_time", "retention_time"]).index
    )


@pytest.mark.parametrize("a_none,b_none", [(True, True), (True, False), (False, True)])
...
def test_tolerance(ms1_peaks):
    a, b = deimos.alignment.tolerance(
        ms1_peaks,
        ms1_peaks,
        dims=["mz", "drift_time", "retention_time"],
        tol=[5e-6, 0.015, 0.3],
...
def test_tolerance_pass_none(ms1_peaks, a_none, b_none):
    ...
    a_, b_ = deimos.alignment.tolerance(
        a,
        b,
        dims=["mz", "drift_time", "retention_time"],
        tol=[5e-6, 0.015, 0.3],
